...
def generate_report(options, report_data, questions, question_key_list, report_name):
    ...
    if options.average == True:
        ...
        for key in question_key_list:
            total = report_data.loc[questions[key],[0]].sum()
            count = report_data.loc[questions[key],["SD", "D", "N", "A", "SA"]].sum()
            report_data.loc[questions[key], "CNT"] = count
            report_data.loc[questions[key], "AVG"] = total / count
    ...
    if options.average == True:
        total = report_data.loc[:,["AVG"]].sum()
        total_avg = total / len(question_key_list)
        print(report_data)
        print(f'Total score: {float(total_avg)}')
        # Remove AVG / CNG
        report_data.drop('AVG', axis=1,inplace=True)
        report_data.drop('CNT', axis=1,inplace=True)
...
def main(options):
    ...
    """
    NOTE: we have in the first column still the "Reports" need to check and remove
    TODO: make proper checks
    We do not call
    """
    report_data = report_data.drop(columns=[(0,0)])
    generate_report(options, report_data,questions, question_key_list, report_name)
